...
<div class="full-box page-header">
	...
	<p class="text-justify">
		Bienvenido a la sección de Actualizar Usuario de nuestro completo Panel de Administrador. Aquí, tienes el control total para gestionar y mantener la información de los usuarios de manera eficiente y precisa.
		En esta sección, puedes acceder y modificar los detalles de cada usuario registrado en nuestra plataforma, asegurando que los perfiles estén siempre actualizados y reflejen la información más reciente.
		Nuestra interfaz intuitiva te permite navegar sin esfuerzo a través de los perfiles de usuario y realizar ajustes necesarios de manera rápida.
		Mantén la base de datos actualizada y garantiza una experiencia óptima para cada usuario al asegurarte de que su información esté siempre al día. Explora esta sección para ejercer un control total sobre la gestión de usuarios y garantizar que tu plataforma opere con la máxima precisión y profesionalismo.
	</p>
</div>
...
<div class="container-fluid">
	...
	?>
		<form class="form-neon FormularioAjax" action="<?php echo SERVERURL; ?>ajax/usuarioAjax.php" method="POST" data-form="update" autocomplete="off">
			<input class="form-control" type="hidden" readonly name="usuario_id_up" id="usuario_id" value="<?php echo $pagina[1]; ?>" required />
			<fieldset>
					...
					<div class="row">
						<div class="col-12 col-md-4">
							<div class="form-group">
								...
								<input type="text" pattern="[0-9-]{8,20}" class="form-control" name="usuario_dni_up" id="usuario_dni" maxlength="20" value="<?php echo $campos['usuario_dni']; ?>" required />
							</div>
						</div>
						...
						<div class="col-12 col-md-6">
							<div class="form-group">
								...
								<input type="text" pattern="[0-9()+]{10,10}" class="form-control" name="usuario_telefono_up" id="usuario_telefono" maxlength="20" value="<?php echo $campos['usuario_telefono']; ?>" required />
							</div>
						</div>
						<div class="col-12 col-md-6">
							<div class="form-group">
								<label for="usuario_direccion" class="bmd-label-floating">Dirección</label>
								<input type="text" pattern="[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,190}" class="form-control" name="usuario_direccion_up" id="usuario_direccion" maxlength="190" value="<?php echo $campos['usuario_direccion']; ?>" required />
							</div>
						</div>
					</div>
				</div>
			...
			<?php if($lc->encryption($_SESSION['id_spm'])!=$pagina[1]){ ?>
				<input class="form-control" type="hidden" readonly name="tipo_cuenta" id="tipo_cuenta" value="Impropia" required />
			<?php }else{ ?>
				<input class="form-control" type="hidden" readonly name="tipo_cuenta" id="tipo_cuenta" value="Propia" required />
			<?php }?>
