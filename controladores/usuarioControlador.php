...
class usuarioControlador extends usuarioModelo {
	...
	public function agregar_usuario_controlador(){
		...
		/* Comprobar el registro del usuario a la BD */
	...
	public function paginador_usuario_controlador($pagina, $registros, $privilegio, $id, $url, $busqueda){
				...
				$tabla .= '<tr class="text-center" >
						...
						<td>
							<form class="FormularioAjax" action="'.SERVERURL.'ajax/usuarioAjax.php" method="POST" data-form="delete" autocomplete="off">
								<input class="form-control" type="hidden" readonly name="usuario_id_del" id="usuario_id" value="'.mainModel::encryption($rows['usuario_id']).'" required />
		...
		if($total>=1 && $pagina<=$Npaginas){
			$tabla .= '<p class="text-right">Mostrando usuarios del '.$reg_inicio.' al '.$reg_final.' de un total de '.$total.'</p>';
